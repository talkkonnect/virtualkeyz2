...
- **Credentials:** valid PINs are loaded from SQLite table **`access_pins`** (`pin`, optional `label`, `enabled`). If no row matches, **`device.fallback_access_pin`** in `virtualkeyz2.json` is accepted when non-empty (same behaviour as the former hard-coded lab PIN; empty disables). Add rows with `sqlite3 access_control.db` or your own tool.
